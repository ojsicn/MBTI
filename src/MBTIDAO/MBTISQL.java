    ...
    public String classify(List<Integer> list) {
        ...
        while (run) {
            ...
            if(j == 0) {
                ...
                System.out.print("답변선택 (1 또는 2) 뒤로가기 3 >> ");
            }
            ans = sc.next();
            switch (ans) {
                case "1":
                ...
                case "3":
                    if ((list.get(j-1) == 1 || list.get(j-1) == 2 || list.get(j-1) == 3)) {
                        cntEI--;
                        j--;
                    } else if ((list.get(j-1) == 4 || list.get(j-1) == 5 || list.get(j-1) == 6)) {
                        cntNS--;
                        j--;
                    } else if ((list.get(j-1) == 7 || list.get(j-1) == 8 || list.get(j-1) == 9)) {
                        cntFT--;
                        j--;
                    } else if ((list.get(j-1) == 10 || list.get(j-1) == 11 || list.get(j-1) == 12)) {
                        cntPJ--;
                        j--;
                    }
                    break;
                default:
                    System.out.println("1, 2 또는 3 중에 입력하세요...");
                    break;
            }
        }
